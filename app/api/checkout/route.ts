...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.eventId || !body.tickets || body.tickets.length === 0) {
      return NextResponse.json({ error: "Missing required fields: eventId and tickets are required" }, { status: 400 })
    }
    ...
    for (const ticket of body.tickets) {
      if (!ticket.stripePriceId || !ticket.quantity || ticket.quantity <= 0) {
        return NextResponse.json(
          { error: "Invalid ticket data: each ticket must have a valid price ID and quantity" },
          { status: 400 },
        )
      }
    }
    ...
    const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = body.tickets.map((ticket) => ({
      price: ticket.stripePriceId,
      quantity: ticket.quantity,
    }))

    // Get the origin for redirect URLs
    const origin = req.nextUrl.origin || process.env.NEXT_PUBLIC_DOMAIN || "http://localhost:3000"
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      custom_text: {
        submit: {
          message: "Tickets will be sent to your email after payment confirmation.",
        },
      },
