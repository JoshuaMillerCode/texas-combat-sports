  ...
  try {
    ...
    for (const item of body.items) {
      if (!item.stripePriceId || !item.quantity || item.quantity <= 0) {
        return NextResponse.json(
          { error: "Invalid item data: each item must have stripePriceId and valid quantity" },
          { status: 400 },
        )
      }
      ...
      if (!item.stripePriceId.startsWith("price_")) {
        return NextResponse.json({ error: "Invalid Stripe price ID format" }, { status: 400 })
      }
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        total_quantity: body.items.reduce((sum, item) => sum + item.quantity, 0).toString(),
